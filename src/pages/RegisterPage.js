...
export default function Login() {
  ...
  // --- LÍNEAS AGREGADAS PARA MANEJAR ESTADOS ---
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [confirmPassword, setConfirmPassword] = useState("");
  // ---------------------------------------------

  // --- FUNCIÓN AGREGADA PARA REGISTRO ---
  const handleRegister = async (e) => {
    e.preventDefault(); // Evita el comportamiento predeterminado de recarga del formulario
    ...
    try {
      ...
      // Si el registro es exitoso, navega a la siguiente ruta
      navigate("/register-two");
      console.log("Usuario registrado con éxito!");
    } catch (error) {
      // Muestra un error si el registro falla (ej: email ya en uso, contraseña débil)
      ...
      <div className="login-container">
        ...
        <form onSubmit={handleRegister}>
          ...
          <p className="register-text">
            Ya tienes cuenta? <a onClick={() => navigate("/login")}>Inicia Sesion</a>
          </p>
        </form>
      </div>
